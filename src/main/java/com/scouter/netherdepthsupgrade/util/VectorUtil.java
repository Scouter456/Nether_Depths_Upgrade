...
public final class VectorUtil {
    ...
    public static Vec3 movementInputToVelocity(Vec3 movementInput, float speed, float yaw) {
        ...
        LOGGER.info("vecbegin " + movementInput);
        if (d < 1.0E-7) {
            return Vec3.ZERO;
        }
        ...
        LOGGER.info("yvec " + vec3d.y);
        LOGGER.info("vec " + new Vec3(vec3d.x * (double)g - vec3d.z * (double)f, vec3d.y, vec3d.z * (double)g + vec3d.x * (double)f));
